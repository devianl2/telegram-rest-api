...
// If a session is not in the pool (e.g. after server restart), callers must
// reconnect using the session string retrieved from the database.
export class TelegramSessionPool {
  ...
  async resolve(sessionId: string): Promise<{ client: TelegramClientService; fromPool: boolean }> {
